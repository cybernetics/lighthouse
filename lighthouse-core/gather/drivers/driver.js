...
class Driver {
   ...
   * has gone idle, or
   * - MAX_WAIT_FOR_LOAD_EVENT milliseconds have passed.
   * See https://github.com/GoogleChrome/lighthouse/issues/627 for more.
   * @param {number} pauseAfterLoadMs
   * @return {!Promise}
   * @private
   */
  _waitForLoad(pauseAfterLoadMs) {
    ...
    // Last resort timeout. Resolves MAX_WAIT_FOR_LOAD_EVENT ms from now on
    // cleanup function that removes loadEvent and network idle listeners.
    const maxTimeoutPromise = new Promise((resolve, reject) => {
      maxTimeoutHandle = setTimeout(resolve, MAX_WAIT_FOR_LOAD_EVENT);
    }).then(_ => {
      return function() {
   ...
   * so the returned promise will only resolve after the MAX_WAIT_FOR_LOAD_EVENT
  ...
  gotoURL(url, options) {
    ...
    return this.sendCommand('Page.enable')
      ...
      .then(_ => waitForLoad && this._waitForLoad(pauseAfterLoadMs));
  }
